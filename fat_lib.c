...
error_code_t deleteFile(char* filename) {
    error_code_t status = ERROR_OK;
    DirectoryEntry_t entry;
    if (s_pHEAD->clusterEntry == 0) {
        status = findNameInRoot(&s_gBootSector, filename, &entry, FILE_ATTRIBUTE_TYPE);
        if (status == ERROR_OK)
        {
            uint32_t startCluster = entry.startCluster;
            entry.name[0] = 0xE5U;
            while (startCluster != EOF_CLUSTER &&
                   startCluster != BAD_CLUSTER &&
                   startCluster != RESERVED_CLUSTER &&
                   startCluster != FREE_CLUSTER)
            {
                uint32_t nextCluster = getNextCluster(&s_gBootSector, startCluster);
                uint8_t sc = freeClusterUsed(startCluster, &s_gBootSector);
                if (sc != ERROR_OK) {
                    status = ERROR_INVALID_CLUSTER;
                    break;
                }
